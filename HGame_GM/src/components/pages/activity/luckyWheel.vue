    ...
    <Row>
      ...
      <Modal v-model="editModal" @on-ok="tableEditClick(tableRowData)">
        ...
        <div style="text-align:center">
          <Form :label-width="80">
            <Form-item label="vip等级列表">
              <Input placeholder="请输入" v-model="tableRowData.vipStr"></Input>
            </Form-item>
            <Form-item label="消耗列表">
              <Input placeholder="请输入" v-model="tableRowData.consumeStr"></Input>
            </Form-item>
            <Form-item label="参与次数">
              <Input placeholder="请输入" v-model="tableRowData.joinTimes"></Input>
            </Form-item>
...
export default {
  ...
  methods: {
    ...
    getAddListData(data) {
      var _this = this
      // 请求数据
      var res = _this.apiList3.api_spirityluckcontroller_loadall(data)
    ...
    tableEditClick(val) {
      ...
      if (this.addOrEdit === 2) {
        console.log(val2)
        var resE = this.apiList3.api_spirityluckcontroller_addorupdate(val2)
        if (resE && resE.code === 0) {
      ...
      if (this.addOrEdit === 1) {
        console.log(val2)
        var resA = this.apiList3.api_spirityluckcontroller_addorupdate(val2)
        if (resA && resA.code === 0) {
    ...
    show(index) {
      this.$Modal.info({
        ...
        content: `id：${this.listDataShow[index].id}<br>
                    ...
                    参与次数：${this.listDataShow[index].joinTimes}<br>
                    消耗列表：${this.listDataShow[index].consumeStr}<br>
                    奖励列表：${this.listDataShow[index].rewardStr}<br>
                    vip等级列表：${this.listDataShow[index].vipStr}<br>
    ...
    openClose(params) {
      ...
      console.log(o, "o")
      var res = this.apiList3.api_spirityluckcontroller_openorclose(o)
      if (!res) {
    ...
    luckReload(params) {
      ...
      var res = this.apiList3.api_spirityluckcontroller_reload(o)
      if (!res) {
    ...
    deleteBatch() {
      ...
      var res = this.apiList3.api_spirityluckcontroller_delete(o)
      if (!res) {
