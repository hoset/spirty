  ...
  <div class="list-lb">
    <Row>
      ...
      <Card>
        ...
        <!--table-->
        <Table class="content-table" v-on:on-select="luckReload" :loading="loading2" :show-header="showHeader" :size="tableSize" :data="listDataShow" :columns="columnsSet" />
        <!--分页-->
      ...
      <Modal v-model="editModal" @on-ok="tableEditClick(tableRowData)">
        ...
        <div style="text-align:center">
          <Form :label-width="80">
            ...
            <Form-item label="自定义商品ID">
              <Input placeholder="请输入" v-model="tableRowData.rechargeNum"></Input>
            </Form-item>
            <Form-item label="奖励列表">
              <Input type="textarea" :autosize="true" placeholder="Enter something..." v-model="tableRowData.rewardStr"></Input>
            </Form-item>
...
export default {
          ...
          render: (h, params) => {
            return h('div', [
              ...
              h('Button', {
                ...
                on: {
                  click: () => {
                    this.openClose(params.row) // 权限操作
                  }
                }
              }, '开/关'),
              h('Button', {
  ...
  methods: {
    ...
    getAddListData(data) {
      var _this = this
      // 请求数据
      var res = _this.apiList3.api_spiritywishcontroller_loadall(data)
    ...
    pageChange(page) {
      ...
      if (page > getPageNum) {
        // 请求数据
        var res = _this.apiList3.api_spiritywishcontroller_loadall({ currentPage: _this.setPageData })
        if (!res || res.hasOwnProperty('code')) {
          this.$Message.error(res.message)
          this.loading2 = false
          return
    ...
    tableEditClick(val) {
      ...
      if (this.addOrEdit === 2) {
        console.log(val2)
        var resE = this.apiList3.api_spiritywishcontroller_addorupdate(val2)
        if (resE && resE.code === 0) {
      ...
      if (this.addOrEdit === 1) {
        console.log(val2)
        var resA = this.apiList3.api_spiritywishcontroller_addorupdate(val2)
        if (resA && resA.code === 0) {
    ...
    show(index) {
      this.$Modal.info({
        ...
        content: `id：${this.listDataShow[index].id}<br>
                    ...
                    自定义商品ID：${this.listDataShow[index].rechargeNum}<br>
                    奖励列表：${this.listDataShow[index].rewardStr}<br>
    ...
    openClose(params) {
      let o = {
        ...
        open: params.open === '是' ? 0 : 1
      }
      console.log(o);
      var res = this.apiList3.api_spiritywishcontroller_openorclose(o)
      if (!res) {
    ...
    luckReload(r) {
      ...
      var res = this.apiList3.api_spiritywishcontroller_reload(o)
      if (!res) {
    ...
    deleteBatch() {
      ...
      var res = this.apiList3.api_spiritywishcontroller_delete(o)
      if (!res) {
