...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getAddListData (page) {
        // 请求数据
        var res = this.apiList3.api_findarpuandarppulog(page)
        ...
        for (var i = 0; i < res.rows.length; i++) {
          ...
          res.rows[i].arpu = (res.rows[i].totalRecharge / res.rows[i].activityRole).toFixed(2)
          res.rows[i].arppu = (res.rows[i].totalRecharge / res.rows[i].rechargeRole).toFixed(2)
        }
