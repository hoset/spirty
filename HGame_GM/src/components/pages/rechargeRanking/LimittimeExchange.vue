<template>
  <div>
    ...
    <Button icon="ios-search" v-on:click="editModal('新增限时兑换活动')">新增</Button><br/>
    重载类型：
    ...
    <Modal v-model="tableModal.open" loading v-on:on-ok="updateOrCreate" :mask-closable="false" :width="890">
      ...
      <Form :model="formData" label-position="left" inline :label-width="100">
        ...
        <FormItem >
          服务器ID：
          <Select placeholder="选择服务器id" transfer v-model="formData.serverName" style="width:200px">
            <Option v-for="item in serverIdItems" :value="item['id']" :key="item['id']">{{ item['text'] }}</Option>
          </Select>
        </FormItem>
        <FormItem>
        ...
        <FormItem>
          ...
          时间类型：<Input v-model="formData['timeType']" placeholder="timeType..." style="width: auto" />
        </FormItem>
        <FormItem>
        ...
        <FormItem>
          ...
          活动状态：<Input v-model="formData['open']" placeholder="open..." style="width: auto" />
        </FormItem>
      </Form>
    </Modal>
  </div>
...
export default {
  ...
  methods: {
    reloadActivity() {
      ...
      this.apiList4._get_(window.apiUrl.api_reloadLimittimeExchange, this.seachParams, (e) => this.$Message.error("操作失败：" + e), (e) => {
    ...
    updateOrCreate() {
      this.apiList4._postj_(window.apiUrl.api_addOrUpdateLimittimeExchange, this.formData, (e) => this.$Message.error("操作失败：" + e), (e) => {
        this.$Message.success("操作成功");
    ...
    queryItems() {
      ...
      this.apiList4._get_(window.apiUrl.api_LimittimeExchange, check(this.seachParams), (e) => console.log(e), (e) => this.datas = e);
    },
    validateSeachParam() {
